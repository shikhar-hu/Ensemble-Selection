...
for(i in rows)
{
	...
	SVMLinear_parameters=expand.grid(degree = c(2,3), cost = 2^seq(-10,4,2))
		# lambda	
...
pb <- winProgressBar(title = paste("SVML progress bar for ",ds[i,"name"]), min = 0, max = length(myFiles), width = 400)

	# for (j in 1:10)
	for (j in 1:length(myFiles))
	{
		# for (k in 1:10)
		for (k in 1:nrow(SVMLinear_parameters))
		{
			...
			set.seed(3456)
			SVMLinear_model <- svm(x = a[,-c(1:2)], y = (a[,2]), scale = TRUE, type = NULL, kernel ="linear", degree = SVMLinear_parameters[k,1], gamma = if (is.vector(x)) 1 else 1 / ncol(x),coef0 = 0, cost = SVMLinear_parameters[k,2], nu = 0.5,class.weights= NULL, cachesize = 40, tolerance = 0.001, epsilon = 0.1,shrinking = FALSE, cross = 0, probability = FALSE, fitted = FALSE)


			val_data$predictions = predict(SVMLinear_model, val_data[,-c(1:2)])
			...
			test_data= read.csv(paste(preproc_dir,'/',"test.csv",sep=''),as.is=T)
			colnames(test_data)<-paste(gsub(".","",colnames(test_data),fixed=TRUE))
			test_data$predictions = predict(SVMLinear_model, test_data[,-c(1:2)])
			...
			{
				val_local=df
				colnames(val_local)[2]<-paste("svml_val",j,k,sep="_")
				test_local=df2
				colnames(test_local)[2]<-paste("svml_test",j,k,sep="_")	
			}else
			{
				val_local=cbind(val_local,z=df$predictions)
				colnames(val_local)[ncol(val_local)]<-paste("svml_val",j,k,sep="_")
				test_local=cbind(test_local,df2$predictions)
				colnames(test_local)[ncol(test_local)]<-paste("svml_test",j,k,sep="_")
			}
	...
	if(j==1)
		...
		setWinProgressBar(pb,j, title=paste(ds[i,"name"]," SVML :", round(j/length(myFiles)*100, 0),"% done"))

	}

setwd(candidates_val)
write.csv (val_global[, -grep("id", colnames(val_global))],"svmlinear_val_all.csv",row.names=F)


setwd(candidates_test)
write.csv (test_global[, -grep("id", colnames(test_global))],"svmlinear_test_all",row.names=F)
close(pb)
