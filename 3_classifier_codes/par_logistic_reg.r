...
pb <- winProgressBar(title = paste("LogR progress bar for ",ds[i,"name"]), min = 0, max = length(myFiles), width = 400)
...
# for (j in 1:5)
	for (j in 1:length(myFiles))
	{
		...
		lr_model <- foreach(k = 1:nrow(lr_parameters),.packages="stepPlr") %dopar% {
			...
			lr_model <- plr(x = a[,-c(1:2)],y = as.numeric(a[,2]),lambda=lr_parameters[k,1], cp=lr_parameters[k,2])
						}
		stopCluster(cl)
		...
		lr_val <- foreach(k = 1:nrow(lr_parameters), .combine='cbind', .packages="stepPlr") %dopar% {
				predict(lr_model[[k]],val_data[,-c(1:2)],type="response")
		...
		lr_test <- foreach(k = 1:nrow(lr_parameters), .combine='cbind', .packages="stepPlr") %dopar% {
				predict(lr_model[[k]],test_data[,-c(1:2)],type="response")
						}                
		stopCluster(cl)
	...
	setWinProgressBar(pb,j, title=paste(ds[i,"name"]," LogR :", round(j/length(myFiles)*100, 0),"% done"))
