...
GeneratePopulation <- function(popsize,dim,low,up)
{
	...
	for (o in 1:popsize)
	{
		for (y in 1:dim)
		{
			pop[o,y]=runif(1,0,1)*(up[y]-low[y])+low[y]
		}
	}
	pop
}
...
bsa=function(fnc,mydata,popsize,dim,DIM_RATE,low,up,n_iter,ds,i)
{
	library(ROCR)
	...
	pop=GenPop(popsize,dim,low,up)  #see Eq.1 in [1] of original paper
	...
	pb <- winProgressBar(title = "BSA progress bar", min = 0, max = n_iter, width = 300)
	...
	for (epk in 1:n_iter)
	{
		...
		if (runif(1,0,1)<runif(1,0,1))
			...
			{
				u=sample(dim) 
				map[q,u[1:ceiling(DIM_RATE*runif(1,0,1)*dim)]]=1  ##Shikhar NOTE - DIM_RATE is speed of backsearch? 1 or less
			} else 
		{
			for (q in 1:popsize)
			{
				map[q,sample(dim)[1]]=1
			}
		}
		...
		if(fitnessoffsprings-fitnesspop >= 0.0001 )	### if AUC increases by this limit then the new offspring becomes globaloptimum
		{
		...
		Runtime=as.numeric(end.time - start.time,units="secs")

		out[epk,]=c(epk,fitnesspop ,Runtime)
		setWinProgressBar(pb, epk, title=paste(ds[i,"name"],": ", round(epk/n_iter*100, 0),"% done - BSA"))
